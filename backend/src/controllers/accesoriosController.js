...
exports.crear = async (req, res) => {
  ...
  if (isNaN(precioNum) || precioNum <= 0) {
    return res
      .status(400)
      .json({ error: "El precio unitario debe ser mayor a cero" });
  }

  const { rows: existe } = await pool.query(
    "SELECT id FROM accesorios WHERE LOWER(nombre) = LOWER($1) AND activo = TRUE",
    [nombre],
  );
  if (existe.length > 0) {
    return res
      .status(409)
      .json({ error: `Ya existe un accesorio con el nombre "${nombre}"` });
  }
...
exports.actualizar = async (req, res) => {
  ...
  const { rows: existe } = await pool.query(
    "SELECT id FROM accesorios WHERE LOWER(nombre) = LOWER($1) AND activo = TRUE AND id != $2",
    [nombre, id],
  );
  if (existe.length > 0) {
    return res
      .status(409)
      .json({ error: `Ya existe otro accesorio con el nombre "${nombre}"` });
  }
...
exports.eliminar = async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query("UPDATE accesorios SET activo = FALSE WHERE id = $1", [
      id,
    ]);
...
exports.stats = async (req, res) => {
  try {
    const [total, stockBajo, movRecientes] = await Promise.all([
      ...
      pool.query(
        "SELECT COUNT(*) FROM accesorios WHERE activo = TRUE AND stock_actual <= stock_minimo",
      ),
      pool.query(`
...
exports.filtrar = async (req, res) => {
  ...
  if (ubicacion_id) {
    ...
    query += ` AND a.ubicacion_id = $${params.length}`;
  }
