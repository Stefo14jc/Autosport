...
export default function Accesorios() {
  ...
  const handleQrScaneado = useCallback(
    (id) => {
      setScannerOpen(false);
      const idBuscado = String(id);
      const rep = accesorios.find(
        (r) => String(r.id) === idBuscado || r.codigo === idBuscado,
      );
      if (rep) {
        setStockModal(rep);
        setMovForm({ tipo: "ingreso", cantidad: "", motivo: "" });
      } else {
        alert("Accesorio no encontrado en el inventario actual");
      }
    },
    [accesorios],
  );
            ...
            <table className="data-table">
              ...
              <tbody>
                ...
                {accesoriosFiltrados.length === 0 && (
                  <tr>
                    <td colSpan={7} className="rep-empty">
