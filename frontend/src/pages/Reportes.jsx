...
export default function Reportes() {
  ...
  useEffect(() => {
    api
      .get("/movimientos/dashboard-diario")
  ...
  const exportarPDF = async () => {
    ...
    doc.text(
      `Total ingresos: ${totalIngresos} uds | Total salidas: ${totalSalidas} uds`,
      14,
      32,
    );
    ...
    const nombreArchivo = `Reporte_AUTOSPORT_${fechaInicio}_${fechaFin}.pdf`;
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);

    // Si estamos en celular y el navegador soporta compartir archivos nativamente (iOS / Android)
      ...
      try {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({
            files: [file],
            title: "Reporte AUTOSPORT",
            text: `Reporte de Movimientos (${fechaInicio} a ${fechaFin})`,
          });
          return;
        }
      } catch (err) {
        // Si el usuario cancela la hoja de compartir o no es compatible, continua al fallback
      }
    }

    // Fallback para PC o navegadores móviles sin Web Share:
    if (isMobile) {
      const pdfBlob = doc.output("blob");
      ...
      <div className="page__body">
        ...
        {diario && (
          <div className="dash-diario">
                    ...
                    {diario.porUsuario.map((u, i) => (
                      <tr key={i}>
                        ...
                        <td data-label="Último registro"
                          ...
                          {new Date(u.ultimo_movimiento).toLocaleTimeString(
                            "es-EC",
                          )}
                        </td>
                      </tr>
            ...
            {diario.porUsuario.length === 0 && (
              <p className="dash-diario__vacio">
        ...
        <div className="rep-filtros">
          ...
          <button
            className="btn btn--primary rep-filtros__btn"
            onClick={consultar}
            disabled={loading}>
